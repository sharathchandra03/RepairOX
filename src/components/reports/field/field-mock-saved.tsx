...
/* ──────────────────────────────────────────────────────────────────────────
   RepairOX — Reports V2 · Field Management · Saved Reports (UI preview)
   ──────────────────────────────────────────────────────────────────────────
   Same card grid + detail pane as Shop's SavedReportsPanel, showing static
   sample reports. Wire to the real saved-reports store once Field engine is
   connected.
   ────────────────────────────────────────────────────────────────────────── */
...
import { fieldSavedReports } from "./mock-data";
...
export function FieldMockSaved({ filterMode = "all" }: { filterMode?: FilterMode }) {
  const [selectedId, setSelectedId] = useState<string | null>(fieldSavedReports[0]?.id ?? null);

  const filtered = useMemo(() => {
    if (filterMode === "pinned") return fieldSavedReports.filter((r) => r.pinned);
    if (filterMode === "recent") return fieldSavedReports.slice(0, 3);
    return fieldSavedReports;
  }, [filterMode]);
            ...
            >
              <div className="flex flex-col items-center justify-center py-12 text-center">
                ...
                <p className="mt-1 max-w-xs text-[12px] text-muted-foreground">
                  Once the Field data engine is connected, this report will render live charts and tables here.
