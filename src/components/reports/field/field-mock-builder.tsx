...
/* ──────────────────────────────────────────────────────────────────────────
   RepairOX — Reports V2 · Field Management · Custom Report Builder (preview)
   ──────────────────────────────────────────────────────────────────────────
   Same step-driven UX as the live Shop builder: Source → Fields → Grouping →
   Metric → Visualization → Save. Uses mock data sources from mock-data.ts —
   swap with real Field engine later (no UI changes needed).
   ────────────────────────────────────────────────────────────────────────── */
...
import { fieldBuilderSources } from "./mock-data";
...
export function FieldMockBuilder() {
  const [sourceId, setSourceId] = useState(fieldBuilderSources[0].id);
  const source = fieldBuilderSources.find((s) => s.id === sourceId) ?? fieldBuilderSources[0];
  ...
  const onSourceChange = (id: string) => {
    const s = fieldBuilderSources.find((x) => x.id === id) ?? fieldBuilderSources[0];
    setSourceId(id);
    ...
    <div className="space-y-4">
      ...
      <div className="grid gap-4 lg:grid-cols-[340px_1fr]">
        ...
        <div className="space-y-3">
          <Panel title="Build a Report" subtitle="Compose from Field data sources (preview)">
            <div className="space-y-3.5">
              <Step n={1} label="Data Source">
                <RSelect
                  value={sourceId}
                  onChange={onSourceChange}
                  options={fieldBuilderSources.map((s) => ({ label: s.label, value: s.id }))}
                />
              </Step>
