...
import {
  ArrowLeft, Sparkles, Filter, Inbox, Phone, Mail, MessageSquare, Target, TrendingUp,
...
const CARDS: Record<string, { name: string; src: string; device: string; value: number; age: string; score: number }[]> = {
  new: [
    { name: "Aarav Mehta",   src: "Google",    device: "iPhone 14 Pro",   value: 9500,  age: "12m", score: 86 },
    { name: "Bina K. Soni",  src: "Meta",      device: "MacBook Air M2",  value: 18000, age: "23m", score: 72 },
    { name: "Chetan Bhatt",  src: "Walk-In",   device: "iPad Pro 11\"",   value: 6500,  age: "45m", score: 64 },
  ],
  contacted: [
    { name: "Diya Sen",      src: "Reference", device: "iPhone 13",       value: 7000,  age: "1h",  score: 78 },
    { name: "Eshan Roy",     src: "YouTube",   device: "iWatch S8",       value: 3500,  age: "2h",  score: 55 },
  ],
  quoted: [
    { name: "Falguni Patel", src: "Google",    device: "iPhone 12",       value: 11000, age: "3h",  score: 90 },
  ],
  won: [
    { name: "Gaurav Pillai", src: "Reference", device: "iMac 24\"",       value: 22000, age: "today", score: 95 },
  ],
};

const SOURCES = [
  { name: "Google",    leads: 38, won: 12, spend: 24000, color: "bg-sky-500"     },
  { name: "Meta",      leads: 22, won: 6,  spend: 15000, color: "bg-violet-500"  },
  { name: "YouTube",   leads: 14, won: 3,  spend: 8000,  color: "bg-rose-500"    },
  { name: "Walk-In",   leads: 31, won: 18, spend: 0,     color: "bg-emerald-500" },
  { name: "Reference", leads: 19, won: 12, spend: 0,     color: "bg-amber-500"   },
];
...
/* ---------- page ---------- */
export default function LeadManagementPreview() {
  return (
    <div className="relative min-h-screen overflow-hidden bg-gradient-to-b from-white via-rose-50/20 to-white">
      ...
      <div className="pointer-events-none absolute -top-40 right-10 h-[420px] w-[620px] rounded-full bg-violet-200/30 blur-3xl" />
      ...
      <main className="relative mx-auto max-w-7xl space-y-6 px-4 pb-16 sm:px-6">
        ...
        <section className="rounded-3xl border border-zinc-200 bg-card p-6 shadow-card sm:p-8">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="text-[11px] font-semibold uppercase tracking-wider text-violet-700">Module 2</p>
          ...
          <div className="mt-6 grid grid-cols-2 gap-3 md:grid-cols-4">
            {KPIS.map((k, i) => {
              ...
              const tone = {
                ...
                rose:    "bg-rose-50    text-brand-700   ring-rose-200",
              }[k.tone];
              return (
                <motion.div
                  key={k.label}
        ...
        <section className="grid grid-cols-1 gap-5 xl:grid-cols-[1fr_360px]">
          ...
          <div className="rounded-3xl border border-zinc-200 bg-card p-5 shadow-card sm:p-6">
            ...
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4">
              {STAGES.map((stage, si) => (
                <div
                  key={stage.id}
                  className="flex min-h-[280px] flex-col gap-2 rounded-2xl border border-zinc-200 bg-zinc-50/70 p-3"
                >
          ...
          <aside className="space-y-5">
            ...
            <div className="rounded-3xl border border-zinc-200 bg-card p-5 shadow-card sm:p-6">
              ...
              <Link
                href="#"
                className="mt-4 inline-flex w-full items-center justify-center gap-1 rounded-xl bg-zinc-50 py-2 text-xs font-semibold text-zinc-700 ring-1 ring-zinc-200 hover:bg-zinc-100"
              >
            ...
            {/* Hand-off CTA */}
            <div className="rounded-3xl border border-dashed border-rose-300 bg-rose-50/50 p-5">
              <div className="flex items-start gap-3">
                <span className="grid h-9 w-9 shrink-0 place-items-center rounded-lg brand-gradient text-white">
