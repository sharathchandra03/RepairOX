...
import {
  ArrowUpRight, Truck, MapPin, Navigation, Route, Camera, Clock, Map as MapIcon,
  Briefcase, ClipboardList, ChevronRight, Search, Plus, Phone, MessageSquare,
  AlertTriangle, CheckCircle2, Zap, Filter, Sparkles,
} from "lucide-react";
    ...
    <div className="relative space-y-6 pb-8">
      ...
      <section className="rounded-3xl border border-zinc-200 bg-card p-6 shadow-card sm:p-8">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <p className="text-[11px] font-semibold uppercase tracking-wider text-sky-700">Field Ops</p>
          ...
          <div className="grid grid-cols-2 gap-2.5 sm:flex sm:gap-2">
            <Input iconLeft={<Search className="h-4 w-4" />} placeholder="Find job, tech, area..." className="h-10 rounded-xl border-zinc-200 bg-zinc-50 sm:w-72" />
            <Button className="gap-1.5"><Route className="h-4 w-4" /> Auto-route</Button>
          </div>
        </div>
      ...
      <section className="grid grid-cols-1 gap-5 lg:grid-cols-[1fr_360px]">
        ...
        <aside className="rounded-3xl border border-zinc-200 bg-card p-5 shadow-card sm:p-6">
          <div className="flex items-center justify-between">
            ...
            <Button variant="outline" size="sm" className="gap-1.5"><Filter className="h-3.5 w-3.5" /> All</Button>
          </div>
          <ul className="mt-4 space-y-2.5">
            {TECHS.map((t, i) => (
              <motion.li key={t.name} initial={{ opacity: 0, x: 6 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.04 * i }}
                className="flex items-center gap-3 rounded-xl border border-zinc-200 bg-white p-2.5 transition hover:border-indigo-200 hover:bg-indigo-50/30">
                <div className="relative">
                  <Avatar name={t.name} size={36} />
                  <span className={cn("absolute -bottom-0.5 -right-0.5 h-3 w-3 rounded-full ring-2 ring-white", t.color)} />
                </div>
                ...
                <div className="text-right">
                  ...
                  {t.eta !== "-" && <p className="text-[10px] text-zinc-500">ETA {t.eta}</p>}
                </div>
              </motion.li>
            ))}
          </ul>
      ...
      <section className="rounded-3xl border border-zinc-200 bg-card shadow-card">
        <div className="flex flex-col gap-3 p-5 sm:flex-row sm:items-center sm:justify-between sm:p-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button size="sm" className="gap-1.5"><Plus className="h-3.5 w-3.5" /> New job</Button>
          </div>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full min-w-[820px] text-sm">
            ...
            <tbody>
              {JOBS.map((j, i) => (
                <motion.tr key={j.id} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.04 * i }}
                  className="group border-t border-zinc-200 transition hover:bg-zinc-50/70">
                  <td className="px-5 py-3 font-medium tnum">{j.id}</td>
                  <td className="py-3"><div className="flex items-center gap-2"><Avatar name={j.customer} size={26} /><span className="whitespace-nowrap">{j.customer}</span></div></td>
                  <td className="py-3 whitespace-nowrap text-zinc-600">{j.device}</td>
                  <td className="py-3 whitespace-nowrap text-zinc-600"><span className="inline-flex items-center gap-1"><MapPin className="h-3 w-3 text-indigo-500" /> {j.area}</span></td>
                  <td className="py-3">
                  ...
                  <td className="py-3 pr-5 text-right">
                    {j.sla === "ok" ? (
                      <span className="inline-flex items-center gap-1 rounded-full bg-emerald-50 px-2 py-0.5 text-[11px] font-semibold text-emerald-700 ring-1 ring-emerald-200"><CheckCircle2 className="h-3 w-3" /> On track</span>
                    ) : (
                      <span className="inline-flex items-center gap-1 rounded-full bg-amber-50 px-2 py-0.5 text-[11px] font-semibold text-amber-700 ring-1 ring-amber-200"><AlertTriangle className="h-3 w-3" /> At risk</span>
                    )}
                  </td>
                </motion.tr>
              ))}
        ...
        <div className="flex items-center justify-between border-t border-zinc-200 p-4">
          ...
          <Link href="#" className="inline-flex items-center gap-1 text-sm font-semibold text-brand-700 hover:underline">View full board <ChevronRight className="h-3.5 w-3.5" /></Link>
        </div>
      </section>
          ...
          <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
            {PIPELINE_STAGES.map((s) => (
              <div key={s.id} className="rounded-2xl border border-zinc-200 bg-zinc-50/70 p-3">
                <div className="flex items-center gap-1.5"><span className={cn("h-1.5 w-1.5 rounded-full", s.dot)} /><span className="text-[11px] font-semibold text-zinc-700">{s.label}</span></div>
                <p className="font-display mt-2 text-2xl font-extrabold tnum">{s.count}</p>
                <p className="text-[10px] text-zinc-500">parcels</p>
              </div>
            ))}
          </div>
          <div className="mt-4 flex items-center justify-between rounded-xl border border-dashed border-indigo-300 bg-indigo-50/40 p-3">
            <div className="flex items-center gap-2">
              <span className="grid h-8 w-8 place-items-center rounded-lg brand-gradient text-white"><Zap className="h-4 w-4" /></span>
              <div>
                <p className="text-sm font-semibold">Auto-create job cards</p>
                <p className="text-[11px] text-zinc-600">When a pickup arrives, generate a ticket instantly.</p>
              </div>
            </div>
            ...
            {ZONES.map((z) => (
              <li key={z.name}>
                ...
                <div className="mt-1.5 h-1.5 overflow-hidden rounded-full bg-zinc-100">
                  <div className={cn("h-full rounded-full", z.capacity >= 85 ? "bg-amber-500" : z.capacity >= 60 ? "bg-emerald-500" : "bg-sky-500")} style={{ width: `${z.capacity}%` }} />
                </div>
              </li>
            ))}
          </ul>
...
function CityMap() {
  return (
    <div className="relative h-[360px] overflow-hidden rounded-2xl border border-zinc-200 bg-gradient-to-br from-sky-50 via-white to-indigo-50/40">
      <svg className="absolute inset-0 h-full w-full" viewBox="0 0 100 60" preserveAspectRatio="none">
        <defs>
          ...
          <linearGradient id="route" x1="0" x2="1" y1="0" y2="0">
            ...
            <stop offset="50%" stopColor="#4361EE" stopOpacity="0.7" />
        ...
        <motion.path d="M 12 18 Q 35 12, 50 28 T 88 14" fill="none" stroke="url(#route)" strokeWidth="0.8" strokeDasharray="2 2"
          animate={{ strokeDashoffset: [0, -8] }} transition={{ duration: 1.4, repeat: Infinity, ease: "linear" }} />
      </svg>
      {TECHS.map((t, i) => (
        <motion.div key={t.name} initial={{ opacity: 0, scale: 0.6 }} animate={{ opacity: 1, scale: 1 }}
          transition={{ delay: 0.1 + i * 0.08, type: "spring", stiffness: 220, damping: 18 }}
          style={{ left: `${t.x}%`, top: `${t.y}%` }} className="absolute -translate-x-1/2 -translate-y-1/2">
          {t.status === "On-route" && (
            <motion.span className="absolute left-1/2 top-1/2 h-9 w-9 -translate-x-1/2 -translate-y-1/2 rounded-full border-2 border-emerald-400/60"
              animate={{ scale: [1, 1.6, 1], opacity: [0.7, 0, 0.7] }} transition={{ duration: 2, repeat: Infinity, ease: "easeOut" }} />
          )}
          <div className="group relative">
            <span className={cn("grid h-9 w-9 place-items-center rounded-full text-white shadow-[0_8px_24px_-8px_rgba(15,23,42,0.4)] ring-2 ring-white", t.color)}>
      ...
      {[{ x: 28, y: 22 }, { x: 55, y: 38 }, { x: 75, y: 30 }, { x: 40, y: 60 }].map((p, i) => (
        <motion.div key={i} initial={{ opacity: 0, y: -6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 + i * 0.1 }}
          style={{ left: `${p.x}%`, top: `${p.y}%` }} className="absolute -translate-x-1/2 -translate-y-full">
          <span className="grid h-7 w-7 place-items-center rounded-full bg-white text-indigo-600 shadow-card ring-1 ring-indigo-200">
            <MapPin className="h-3.5 w-3.5 fill-indigo-500 stroke-white" />
          </span>
        </motion.div>
      ))}
      <div className="absolute right-3 top-3 flex flex-col gap-1.5">
        <button className="grid h-8 w-8 place-items-center rounded-lg bg-white/90 text-zinc-700 shadow-card ring-1 ring-zinc-200 backdrop-blur hover:bg-white"><Plus className="h-3.5 w-3.5" /></button>
        <button className="grid h-8 w-8 place-items-center rounded-lg bg-white/90 text-zinc-700 shadow-card ring-1 ring-zinc-200 backdrop-blur hover:bg-white"><Navigation className="h-3.5 w-3.5" /></button>
      </div>
