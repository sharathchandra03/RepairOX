...
import {
  ...
  CUSTOMER_SOURCES, CUSTOMER_SOURCE_LABEL, type Customer, type CustomerSource,
...
export default function ManageCustomersPage() {
  ...
  const { customers, customerGroups, addCustomer, updateCustomer, deleteCustomer, mergeCustomersAction } = useStore();
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        subtitle="Verified customers who have started a service or commercial relationship with your organization."
        actions={
      ...
      {tab === "customers" && (
        <>
            ...
            <div className="overflow-x-auto">
              <table className="w-full table-fixed text-[14px]">
                <colgroup>
                  ...
                  <col className="w-[12%]" />{/* Company / Groups */}
                  <col className="w-[12%]" />{/* Activity */}
                  <col className="w-[10%]" />{/* Lifetime Value */}
                  <col className="w-[6%]" />{/* Loyalty */}
                  <col className="w-[8%]" />{/* Last Visit */}
                ...
                <tbody>
                  {paged.map((c) => {
                    ...
                    return (
                      <tr
                        key={c.id}
                        onClick={() => router.push(`/settings/customers/manage/${c.id}`)}
          ...
          <div className="grid grid-cols-1 gap-3 md:hidden">
            {paged.map((c) => (
              <div key={c.id} onClick={() => router.push(`/settings/customers/manage/${c.id}`)} className="cursor-pointer rounded-2xl border border-border bg-card p-4 shadow-card">
                <div className="flex items-center justify-between">
          ...
          {/* Footer note — clarifies scope: Walk-In placeholders/prospects are
              tracked as Contacts (CRM Contacts tab), never mixed into Customer
              Master, until they're qualified/promoted. */}
          <div className="rounded-xl border border-emerald-200 bg-emerald-50/60 px-4 py-2.5 text-center text-[12px] text-emerald-800">
            <span className="font-medium">"Walk-in Customer" placeholders and prospects are excluded</span> - they remain unlinked transactions or CRM Contacts until qualified.
          </div>
        </>
      )}
